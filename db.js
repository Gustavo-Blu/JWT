const Sequelize = require('sequelize');
const jwt = require('jsonwebtoken')
const { STRING } = Sequelize;
const bcrypt = require('bcrypt')

const config = {
  logging: false
};

if(process.env.LOGGING){
  delete config.logging;
}
const conn = new Sequelize(process.env.DATABASE_URL || 'postgres://localhost/acme_db', config);

const User = conn.define('user', {
  username: STRING,
  password: STRING
});

const Note = conn.define('note', {
  text: STRING
})

User.hasMany(Note)
Note.belongsTo(User)

User.byToken = async(token)=> {
  try {
    const { userId } = jwt.verify(token, process.env.JWT)
    const user = await User.findByPk(userId);
    if(user){
      return user;
    }
    const error = Error('bad credentials');
    error.status = 401;
    throw error;
  }
  catch(ex){
    const error = Error('bad credentials');
    error.status = 401;
    throw error;
  }
};

User.authenticate = async({ username, password })=> {
  const user = await User.findOne({
    where: {
      username
    }
  });

  const isValid = await bcrypt.compare(password, user.password)
  
  if(isValid){
    console.log('i fired')
    const token = jwt.sign({userId: user.id}, process.env.JWT)
    return token; 
  }
  const error = Error('bad credentials');
  error.status = 401;
  throw error;
};

User.beforeCreate(async (user) => {
  const SALT_COUNT = 5;
  user.password = await bcrypt.hash(user.password, SALT_COUNT)
})

const syncAndSeed = async()=> {
  await conn.sync({ force: true });
  const credentials = [
    { username: 'lucy', password: 'lucy_pw'},
    { username: 'moe', password: 'moe_pw'},
    { username: 'larry', password: 'larry_pw'}
  ];
  const notes = [
    {text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.'},
    {text: ' Sit amet tellus cras adipiscing enim eu turpis egestas pretium.'},
    {text: 'dfthdhgfsdghsfgfshgfsgfshfg'},
    {text: 'asdyhgvhasdhyjvaggvaguvgvgajhvdjuasjugsajhkvhjagui'}
  ]

  const [lucy, moe, larry] = await Promise.all(
    credentials.map( credential => User.create(credential))
  );
  const [note1, note2, note3, note4] = await Promise.all(
    notes.map(note => Note.create(note))
  )

  await lucy.setNotes([note1, note4])
  await moe.setNotes([note2])
  await larry.setNotes([note3])

  return {
    users: {
      lucy,
      moe,
      larry
    }
  };
};

module.exports = {
  syncAndSeed,
  models: {
    User
  }
};